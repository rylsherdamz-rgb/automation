...
MIN_CLIP_DURATION = float(os.getenv("LOCAL_MIN_CLIP_DURATION", "30"))  # shortest allowed short
...
def require_nvidia_key() -> str:
    if not NVIDIA_API_KEY:
        raise RuntimeError(
            "NVIDIA_API_KEY is not set. Local mode needs an NVIDIA NIM key when LLM_PROVIDER=nvidia. "
            "Get one at https://build.nvidia.com and add it to your .env."
        )
