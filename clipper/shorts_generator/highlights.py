...
from . import muapi
from .config import MIN_CLIP_DURATION
...
def _enforce_min_duration(highlights: List[Dict], duration: float) -> List[Dict]:
    """Expand clips shorter than MIN_CLIP_DURATION into adjacent content.

    Processes highest-scored clips first so strong clips keep their priority;
    expansion clamps to the source boundaries and never overlaps a clip that
    was already fixed.
    """
    ...
    for h in sorted(highlights, key=lambda x: x["score"], reverse=True):
        ...
        need = MIN_CLIP_DURATION - (end - start)
        if need > 0:
            back_wall = max([p[1] for p in fixed if p[1] <= start] or [0.0])
            take_back = min(need, max(0.0, start - back_wall))
            start -= take_back
            need -= take_back
            if need > 0:
                fwd_wall = min([p[0] for p in fixed if p[0] >= end] or [duration])
                take_fwd = min(need, max(0.0, fwd_wall - end))
                end += take_fwd
            h["start_time"], h["end_time"] = start, end
        result.append(h)
...
def get_highlights(
    ...
    highlights = _enforce_min_duration(highlights, duration)
    return {"highlights": highlights}
