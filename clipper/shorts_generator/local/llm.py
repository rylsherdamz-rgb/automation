"""Local LLM backend — OpenAI, Gemini, or NVIDIA NIM, selected by LLM_PROVIDER."""
from ..config import (
    GEMINI_MODEL,
    LLM_PROVIDER,
    NVIDIA_BASE_URL,
...
def call_nvidia_llm(prompt: str) -> str:
    """NVIDIA NIM (OpenAI-compatible) backend used by --mode local when LLM_PROVIDER=nvidia."""
    try:
        from openai import OpenAI  # type: ignore
    except ImportError as e:
        raise RuntimeError(
    ...
    client = OpenAI(api_key=require_nvidia_key(), base_url=NVIDIA_BASE_URL)
    response = client.chat.completions.create(
        model=NVIDIA_MODEL,
        temperature=0.7,
        messages=[{"role": "user", "content": prompt}],
...
def call_local_llm(prompt: str) -> str:
    ...
    raise RuntimeError(
        f"Unknown LLM_PROVIDER={provider!r}. Use 'openai', 'gemini', or 'nvidia'."
    )
